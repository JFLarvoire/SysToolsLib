...
*                                                                             *
*   History:                                                                  *
*    2001-09-24 JFL Created this file.					      *
*                                                                             *
...
#define NMSGNAMES (sizeof(msgNames) / sizeof(MSGNAME))

static char *GetMsgName(int iMessage, char *pszBuf, int iBufLen) {
  int i;

  for (i=0; i<NMSGNAMES; i++) if (iMessage == msgNames[i].iMsg) { strncpy(pszBuf, msgNames[i].pszName, iBufLen); return pszBuf; }

  wsprintf(pszBuf, "WM_?=0x%04X", iMessage);
  return pszBuf;
}
