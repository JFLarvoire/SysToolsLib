...
endlocal
exit /b &:# %RETURN.ERR% will be processed in the %DEBUG#% macro.
...
endlocal & set "YEAR=%YEAR%" & set "MONTH=%MONTH%" & set "DAY=%DAY%" & set "HOUR=%HOUR%" & set "MINUTE=%MINUTE%" & set "SECOND=%SECOND%" & set "MS=%MS%" & exit /b
...
echo   -n N     Run the command N times. Default: 1 time
...
call :Now
set START_TIME=%HOUR%:%MINUTE%:%SECOND%.%MS%
set START_DATE=%YEAR%-%MONTH%-%DAY%
%>DEBUGOUT% %ECHO.TIME% Started on %START_DATE% at %START_TIME%
...
%>DEBUGOUT% %ECHO.TIME% Ended on %END_DATE% at %END_TIME%
